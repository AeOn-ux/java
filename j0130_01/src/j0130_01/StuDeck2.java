...
public class StuDeck2 {
	StuDeck2(){
		list.add(new Stuscore2("홍길동",100,100,100));
		list.add(new Stuscore2("유관순",100,100,100));
		list.add(new Stuscore2("이순신",100,100,100));
	}
	// 객체컬렉션 - 객체 담을 수 있는 배열
	// add, get, remove, size(), isEmpty()
	ArrayList<Stuscore2> list = new ArrayList();
	...
	// 학생성적입력
		void stuInput() {
			System.out.println((Stuscore2.count+1)+"번 학생 이름을 입력하세요.>>");
			name = scan.next();
			...
			// 추가적으로 번호, 합계, 평균 부여
			list.add(new Stuscore2(name, kor, eng, math)); // list 추가
			System.out.println("======================");
			System.out.println(name+"학생 성적이 추가되었습니다!");
			System.out.println();
...
//==============================================================================
		...
		void stuOutput() {
			...
			// Iterator 사용 출력
			Iterator<Stuscore2> it = list.iterator();
			while(it.hasNext()) {
				Stuscore2 s = it.next();
				System.out.printf("s\t%s\t%s\t%s\t%s\t%s\t%s\n",
